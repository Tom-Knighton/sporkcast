...
public extension FullDBMealplanEntry {
    ...
    func toDomainModel() -> MealplanEntry {
        ...
        if let dbRecipe = self.recipe {
            ...
            recipe = Recipe(id: dbRecipe.id, title: dbRecipe.title, description: dbRecipe.description, author: dbRecipe.author, sourceUrl: dbRecipe.sourceUrl, image: image, timing: .init(totalTime: 0, prepTime: 0, cookTime: 0), serves: nil, ratingInfo: nil, dateAdded: dbRecipe.dateAdded, dateModified: dbRecipe.dateModified, ingredientSections: [], stepSections: [], dominantColorHex: dbRecipe.dominantColorHex, homeId: dbRecipe.homeId)
        }
